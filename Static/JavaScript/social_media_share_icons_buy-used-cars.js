...
function getShareIconSeoText(makeYear, makeName, rootName, cityName) {
  return "Share used " + makeYear + " " + makeName + " " + rootName + " in " + cityName;
}
...
function bind_social_share_icon(share_stage) {
  try {
    ...
    for (var i = 0; i < page_listings.length; i++) {
      ...
      var sh_city = document
        .getElementById("show_social_icons_" + now_listing)
        .getAttribute("data-share-model");
      var shareIconAltTitle = getShareIconSeoText(sh_mfgyear, sh_make, sh_model, sh_city);
